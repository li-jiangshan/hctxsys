    ...
    $.fn.duxTable = function (options) {
        ...
        this.each(function () {
            ...
            function operat(obj, url, success, failure) {
                    ...
                    var dialog = top.layer.confirm('你确认执行' + text + '操作？', function () {
                        var dload = top.layer.load('操作执行中，请稍候...');
                        $.post(url, {
                                data: $(obj).attr('data')
                            },
                            function (json) {
                                top.layer.close(dload);
                        ...
                        if (data.code == 1) {
                            ...
                            if ($.isFunction(options.returnFun)) {
                                ...
                                if (data.url == null || data.url == '') {
									...
									//top.layer.closeAll();
									window.location.reload();
                                    //top.layer.alert(data.info, {skin: 'layui-layer-molv',closeBtn: 0});
                                } else {
                                    //带连接
                                    top.layer({
                                        shade: [0],
            ...
            Do.ready('webuploader', function () {
                ...
                uploader.on('uploadSuccess', function (file, data) {
                    ...
                    if (data.status) {
						...
						$("#qrcodeurl").val(data.data.qrcodeurl);
